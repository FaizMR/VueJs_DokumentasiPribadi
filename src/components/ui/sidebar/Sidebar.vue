...
<template>
  ...
  <div v-if="collapsible === 'none'" data-slot="sidebar"
    :class="cn('bg-transparent text-sidebar-foreground flex h-full w-(--sidebar-width) flex-col', props.class)"
    v-bind="$attrs">
    <slot />
  </div>
  ...
  <Sheet v-else-if="isMobile" :open="openMobile" v-bind="$attrs" @update:open="setOpenMobile">
    ...
    <SheetContent data-sidebar="sidebar" data-slot="sidebar" data-mobile="true" :side="side"
      class="bg-transparent text-sidebar-foreground w-(--sidebar-width) p-0 [&>button]:hidden" :style="{
        '--sidebar-width': SIDEBAR_WIDTH_MOBILE,
      }">
      <SheetHeader class="sr-only">
        <SheetTitle>Sidebar</SheetTitle>
  ...
  <div v-else class="group peer text-sidebar-foreground hidden md:block" data-slot="sidebar" :data-state="state"
    ...
    <div :class="cn(
      ...
      <div data-sidebar="sidebar"
        class="pl-[20%] bg-transparent group-data-[variant=floating]:border-l-slate-200/50 dark:group-data-[variant=floating]:border-l-white/10 flex h-full w-full flex-col  group-data-[variant=floating]:border-l scrollbar-track-transparent">
        <slot />
      </div>
    </div>
  </div>
